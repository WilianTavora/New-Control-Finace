...
db.collection("Entradas").get().then((snapshot)=>{
    const banco = snapshot.docs.reduce((acc, doc)=> {acc + doc.data()
        ...
        li.innerHTML = `
            ...
            <button data-remove="${doc.id}" class="material-icons btndel" style="font-size: 15px;">delete</button>
        `
        listaLis.append(li)
...
listaLis.addEventListener("click", (e)=> {
    const btndelete = e.target.dataset.remove
    if(btndelete){
        db.collection("Entradas").doc(btndelete).delete()
            .then(()=>{
                console.log("Item removido com sucesso")
                location.reload()
            })
            .catch(e =>{
                console.log("Erro ao deletar item", e.message)
            })
    }
